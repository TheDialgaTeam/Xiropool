...
namespace Xiropht_Mining_Pool.Mining
{
    ...
    public class MinerTcpObject : IDisposable
    {
        ...
        private static string LastSubmittedBlock { get; set; }
        ...
        public async void MiningPoolSendJobAsync(decimal jobTarget = 0)
        {
            ...
            if (MiningDifficultyStart < 2)
            {
                MiningDifficultyStart = 2;
            }
        ...
        private async Task GenerateJobAsync(decimal min, decimal max)
        {
            ...
            Console.WriteLine("Job min range selected:" + min);
                ...
                var highestFactor = Math.Log((double)((ClassMiningPoolGlobalStats.CurrentBlockJobMaxRange - 1) * (ClassMiningPoolGlobalStats.CurrentBlockJobMaxRange - 1)), 2);
                var currentFactor = Math.Log((double)((max - 1) * (max - 1)), 2);
                var factor = Math.Floor(highestFactor - currentFactor);

                if (factor < 1)
                    factor = 1;

                int totalRandomJobToFound = (int)factor;
                ...
                while (!jobGenerated || !IsLogged || !IsConnected)
                {
                    ...
                    while (decimal.Parse(firstNumber) < min || decimal.Parse(firstNumber) > max || decimal.Parse(secondNumber) < min || decimal.Parse(secondNumber) > max || resultCalculation < min || resultCalculation > ClassMiningPoolGlobalStats.CurrentBlockJobMaxRange)
                    {
                        if (stopwatch.ElapsedMilliseconds >= 5 * 1000)
                        {
                            break;
                        ...
                        if (ClassUtility.GetRandomBetween(0, 100) >= ClassUtility.GetRandomBetween(0, 100))
                        {
                            firstNumber = ClassUtility.GenerateNumberMathCalculation(min, max, ClassMiningPoolGlobalStats.CurrentBlockDifficulty.Length);
                        }
                        else
                        {
                            firstNumber = ClassUtility.GetRandomBetweenJob(min, max).ToString("F0");
                        }

                        if (ClassUtility.GetRandomBetween(0, 100) >= ClassUtility.GetRandomBetween(0, 100))
                        {
                            secondNumber = ClassUtility.GenerateNumberMathCalculation(min, max, ClassMiningPoolGlobalStats.CurrentBlockDifficulty.Length);
                        }
                        else
                        {
                            secondNumber = ClassUtility.GetRandomBetweenJob(min, max).ToString("F0");
                        }
        ...
        private async void CheckShareHashWithBlockIndicationAsync(decimal result, string mathCalculation, string share, string hash)
        {
            try
            {
                if (LastSubmittedBlock == share)
                    return;

                LastSubmittedBlock = share;
                ...
                ClassLog.ConsoleWriteLog("Miner IP " + Ip + " with Wallet Address: " + MinerWalletAddress + " seems to have found the block " + ClassMiningPoolGlobalStats.CurrentBlockId + $" ({mathCalculation}), waiting confirmation.", ClassLogEnumeration.IndexPoolGeneralLog, ClassLogConsoleEnumeration.IndexPoolConsoleYellowLog, true);
            }
            catch
            {
                ...
                try
                {
                    ...
                    if (IsLogged)
                    {
                        ...
                        if (LastShareReceived + (MiningPoolSetting.MiningPoolIntervalChangeJob * 1000.0m) < ClassUtility.GetCurrentDateInMilliSecond())
                        {
                            ...
                            {
                                if (CurrentMiningJobDifficulty - CurrentMiningJobDifficulty * 0.01m > 1000)
                                {
                                    await Task.Factory.StartNew(() => MiningPoolSendJobAsync(CurrentMiningJobDifficulty - CurrentMiningJobDifficulty * 0.01m), CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Current).ConfigureAwait(false);
                                }
                                else
                                {
                                    await Task.Factory.StartNew(() => MiningPoolSendJobAsync(1000), CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Current).ConfigureAwait(false);
                                }
                            }
                        }
